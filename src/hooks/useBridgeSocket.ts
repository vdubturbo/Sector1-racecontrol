...
function normalizePosition(raw: Record<string, unknown>): NormalizedCarPosition {
  return {
    ...
    overallPosition: Number(raw.overallPosition ?? raw.positionOverall ?? 0),
    classPosition: Number(raw.classPosition ?? raw.positionInClass ?? 0),
    lastLapTime: String(raw.lastLapTime ?? raw.lastLap ?? ''),
    bestTime: String(raw.bestTime ?? raw.bestLapTime ?? ''),
    ...
    driverName: String(raw.driverName ?? (Array.isArray(raw.driverNames) ? raw.driverNames[0] : '') ?? ''),
    ...
    lastLapCompleted: Number(raw.lastLapCompleted ?? raw.lapsCompleted ?? 0),
